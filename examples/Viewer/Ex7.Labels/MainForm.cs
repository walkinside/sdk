...
namespace WIExample
{
    public partial class MainForm : VRForm
    {
        ...
        public MainForm()
        {
            ...
            m_LabelGroup = SDKViewer.LabelManager.CreateLabelGroup();
        }

        // All the label objects owned by this plugin. Stored in a dictionary to find easily the instance based on the ILabel.ID.
        Dictionary<uint, ILabel> m_Labels = new Dictionary<uint, ILabel>();
        ILabelGroup m_LabelGroup = null; // The label group owned by this plugin.
        ...
        void m_ItemDestroy_Click(VRRayCastResult res)
        {
            ...
            if (m_Labels.TryGetValue(res.LabelId, out var label))
            {
        ...
        void m_ItemCreate_Click(VRRayCastResult res)
        {
            ...
            var label = m_LabelGroup.Add("New Label\n" + res.Position.Value.ToString("f2"), (Point3d)res.Position);
