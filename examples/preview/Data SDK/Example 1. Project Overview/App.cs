...
namespace DataSdkExamples
{
    ...
    class App
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (var project = ProjectManager.Open(projectAddress, AccessMode.ReadOnly, creds))
            {
                Console.WriteLine();
                ...
                var projectDirPath = Path.GetDirectoryName(project.ActualFilePath);
                var fileCount = 0;
                long totalSize = 0;
                ...
                Console.WriteLine("ITS objects:");
                //
                var smartObjectCategoriesAndCounts = project.SmartObjects.GetAllCategories()
                    .OrderBy(category => category.Name)
                    .Select(category => string.Format(
                                        "{0} ({1})",
                                        category.Name,
                                        category.GetAllBranches().Count()));
                //
                Console.WriteLine(
                    "  Smart Object categories / counts: {0}",
                    FormatList(smartObjectCategoriesAndCounts));
