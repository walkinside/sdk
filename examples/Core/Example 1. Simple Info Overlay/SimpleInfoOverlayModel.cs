    ...
    sealed class SimpleInfoOverlayModel : CoreSdk.Model
    {
        ...
        public override void Render(CoreSdk.RenderingContext context)
        {
            ...
            var position = new Vector2d(
                x: 10,
                y: context.Renderer.Viewport.Height - 32); // Leaving space for 32 pixel line height.
            var shadowPosition = new Vector2d(
                position.X + 2,
                position.Y - 2);

            // White opaque text with black opaque shadow.
            var color = new Vector4d(1.0, 1.0, 1.0, 1.0);
            var shadowColor = new Vector4d(0.0, 0.0, 0.0, 1.0);
