...
namespace CoreSdkExamples
{
    sealed class CountdownModel : CoreSdk.Model
    {
        ...
        public override void Render(CoreSdk.RenderingContext context)
        {
            ...
            // shrink to 10%.
            var startHeight = context.Renderer.Viewport.Height;
            var endHeight = context.Renderer.Viewport.Height * 0.1;
            ...
            using (context.Renderer.SetRenderingMode(CoreSdk.RenderingMode.Default2d))
            {
                foreach (var m in this.messages)
                {
                    ...
                    var position = new Vector2d(
                        x: (context.Renderer.Viewport.Width - textDim.X) / 2,
                        y: (context.Renderer.Viewport.Height - textDim.Y) / 2);

                    // Red text fading out gradually.
                    var opaqueness = (float)Lerp(1, 0, messageAnimTimeNorm);
                    var color = new Vector4d(
                        m.Color.R / 255.0,
                        m.Color.G / 255.0,
                        m.Color.B / 255.0,
                        opaqueness);
